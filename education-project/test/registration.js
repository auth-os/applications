...
contract('#TestRegistryInit', function(accounts) {
  ...
  describe('#registerTest', async () => {
    ...
    describe('valid registration', async () => {
      ...
      describe('#getRegisteredTests', async() => {
        ...
        it('should return the test that was registered', async () => {
          hexStrEquals(registeredTests[0], 'Test')
        })
      ...
      describe('#getVersionIndex', async () => {
        ...
        beforeEach(async () => {
          versionIndex = await testIdx.getVersionIndex.call(
            storage.address, executionID, name, initial
          ).should.be.fulfilled
        })

        it('should return index 1', async () => {
          versionIndex.toNumber().should.be.eq(1)
        })


      })

      describe('multiple registration', async () => {

        describe('with the same test name', async () => {

          let invalidCalldata

          beforeEach(async () => {
            let invalidCalldata = await testUtils.registerTest.call(name, exec).should.be.fulfilled 
          })
        ...
        describe('with the same initial address', async () => {

          let cd 
          let _events
          let otherName = web3.sha3('Test2')

          beforeEach(async () => {
            let cd = await testUtils.registerTest.call(otherName, initial).should.be.fulfilled 

            _events = await storage.exec(
              exec, executionID, cd,
              { from: exec }
            ).should.be.fulfilled.then((tx) => {
              return tx.logs
            })
            _events.length.should.be.eq(1)
          })

          it('should emit an ApplicationExecution event', async () => {
            _events[0].event.should.be.eq('ApplicationExecution')
          })

        })

        describe('with the same test name and initial address', async () => {

          let invalidCalldata

          beforeEach(async () => {
            let invalidCalldata = await testUtils.registerTest.call(name, initial).should.be.fulfilled 
          })
        ...
        describe('with new information', async () => {
          
          let cd 
          let _events
          let otherInitial = exec
          let otherName = web3.sha3('Test3')
          
          before(async () => {
            let cd = await testUtils.registerTest.call(otherName, otherInitial).should.be.fulfilled 
          ...
          describe('#getRegisteredTests', async() => {
            ...
            it('should return 3 registered tests', async () => {
              registeredTests.length.should.be.eq(3)
            })

            it('should return the correct first test', async () => {
              hexStrEquals(registeredTests[0], 'Test')
            })

            it('should return the correct second test', async () => {
              hexStrEquals(registeredTests[1], 'Test2')
            })

            it('should return the correct first test', async () => {
              hexStrEquals(registeredTests[2], 'Test3')
            })
