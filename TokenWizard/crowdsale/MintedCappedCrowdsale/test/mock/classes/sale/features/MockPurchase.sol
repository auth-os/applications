...
library MockPurchase {
  ...
  function buy() internal view {
    ...
    if (
      uint(Contract.read(MockSale.is_init())) == 0 // Crowdsale is not yet initialized
      || uint(Contract.read(MockSale.is_final())) == 1         // Crowdsale is already finalized
    ...
    if (tier_is_whitelisted) {
      if (Contract.read(MockSale.has_contributed(Contract.sender())) == bytes32(1)) {
        (amount_spent, amount_purchased) = getPurchaseInfo(
          uint(Contract.read(MockSale.decimals())),
          purchase_price,
          tokens_remaining,
          uint(Contract.read(MockSale.whitelist_max_cap(Contract.sender(), current_tier))),
          0,
          tier_is_whitelisted
        );
        ...
        (amount_spent, amount_purchased) = getPurchaseInfo(
          ...
          tokens_remaining,
          uint(Contract.read(MockSale.whitelist_max_cap(Contract.sender(), current_tier))),
          uint(Contract.read(MockSale.whitelist_min_cap(Contract.sender(), current_tier))),
          tier_is_whitelisted
        );
      ...
      if (Contract.read(MockSale.has_contributed(Contract.sender())) == bytes32(1)) {
        ...
        (amount_spent, amount_purchased) = getPurchaseInfo(
          ...
          tokens_remaining,
          0,
          uint(Contract.read(MockSale.min_contribution())),
          tier_is_whitelisted
        );
      }
    }
    ...
    Contract.pay(amount_spent).toAcc(address(Contract.read(MockSale.wallet())));
    ...
    if (Contract.read(MockSale.has_contributed(Contract.sender())) == bytes32(0)) {
      Contract.increase(
        MockSale.contributors()
      ).by(1);
      Contract.set(
        MockSale.has_contributed(Contract.sender())
      ).to(true);
    }
    ...
    if (tier_is_whitelisted) {
      Contract.set(
        MockSale.whitelist_max_cap(Contract.sender(), current_tier)
      ).to(uint(0));
      Contract.set(
        MockSale.whitelist_min_cap(Contract.sender(), current_tier)
      ).to(Contract.read(MockSale.whitelist_max_cap(Contract.sender(), current_tier)));
    }

    // If this tier was updated, set storage 'current tier' information -
    if (updated_tier) {
      Contract.increase(
        MockSale.current_tier()
      ).by(1);
      Contract.set(
        MockSale.ends_at()
      ).to(tier_ends_at);
  ...
  */
  ...
  returns (
    ...
    uint num_tiers = uint(Contract.read(MockSale.crowdsale_tiers()));
    current_tier = uint(Contract.read(MockSale.current_tier())) - 1;
    tier_ends_at = uint(Contract.read(MockSale.ends_at()));
    tokens_remaining = uint(Contract.read(MockSale.tokens_remaining()));
  ...
  */
  ...
  returns (uint purchase_price, bool tier_is_whitelisted) {
    ...
    purchase_price = uint(Contract.read(MockSale.tier_price(current_tier)));
    // Get the current tier's whitelist status
    tier_is_whitelisted = Contract.read(MockSale.tier_is_whitelisted(current_tier)) == bytes32(1) ? true : false;
  }
  ...
  */
  ...
  returns (uint tokens_remaining, uint purchase_price, bool tier_is_whitelisted, uint tier_ends_at) {
    ...
    while (now >= ends_at && ++current_tier < num_tiers) {
      ...
      tokens_remaining = uint(Contract.read(MockSale.tier_sell_cap(current_tier)));
      // Push tier token price storage location to buffer
      purchase_price = uint(Contract.read(MockSale.tier_price(current_tier)));
      // Push tier duration storage location to buffer
      uint tier_duration = uint(Contract.read(MockSale.tier_duration(current_tier)));
