...
library MockTransfer {
  ...
  internal view {
    ...
    Contract.decrease(
      MockToken.balances(Contract.sender())
    ).by(_amt);
    // Update recipient token balance - reverts in case of overflow
    Contract.increase(
      MockToken.balances(_dest)
    ).by(_amt);
  ...
  internal view {
    ...
    Contract.decrease(
      MockToken.allowed(_owner, Contract.sender())
    ).by(_amt);
  ...
  function isTransferAgent() internal view {
    if (
      uint(Contract.read(MockToken.transferAgent(Contract.sender()))) == 0
      && uint(Contract.read(MockToken.tokensUnlocked())) == 0
