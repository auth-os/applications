...
library MockManageTokens {
  ...
  function setTransferAgentStatus(address agent, bool is_agent) internal view { 
    ...
    if (Contract.sender() != address(Contract.read(MockAdmin.admin())))
    ...
    bytes32 status_location = MockAdmin.transfer_agent(agent);
  ...
  function updateMultipleReservedTokens(address[] destinations, uint[] num_tokens, uint[] num_percents, uint[] percent_decimals) internal view { 
    ...
    uint num_destinations = uint(Contract.read(MockAdmin.reserved_destinations()));
    ...
    for (uint i = 0; i < destinations.length; i++) {
      ...
      if (Contract.read(MockAdmin.reserved_info(destinations[i])) == bytes32(0)) {
        ...
        Contract.set(
          bytes32(32 * num_destinations + uint(MockAdmin.reserved_destinations()))
        ).to(to_add);
        // Store reservation info
        Contract.set(
          MockAdmin.reserved_info(to_add)
        ).to(num_destinations);
      }

      // Store reservation info
      Contract.set(
        bytes32(32 + uint(MockAdmin.reserved_info(to_add)))
      ).to(num_tokens[i]);
      Contract.set(
       bytes32(64 + uint(MockAdmin.reserved_info(to_add))) 
      ).to(num_percents[i]);
      Contract.set(
        bytes32(96 + uint(MockAdmin.reserved_info(to_add)))
      ).to(percent_decimals[i]);
    }
    // Finally, update array length
    Contract.set(
      MockAdmin.reserved_destinations()
    ).to(num_destinations);
  ...
  function removeReservedTokens(address destination) internal view { 
    ...
    uint reservation_len = uint(Contract.read(MockAdmin.reserved_destinations()));
    // Get index of passed-in destination. If zero, sender is not in reserved list - revert
    uint to_remove = uint(Contract.read(MockAdmin.reserved_info(destination)));
    ...
    if (to_remove != reservation_len) {
      ...
      address last_index = address(Contract.read(bytes32(32 * reservation_len + uint(MockAdmin.reserved_destinations()))));

      // Update index
      Contract.set(
        MockAdmin.reserved_info(last_index)
      ).to(to_remove);
      // Push last index address to correct spot in reserved_destinations() list
      Contract.set(
        bytes32((32 * to_remove) + uint(MockAdmin.reserved_destinations()))
      ).to(last_index);
    }
    ...
    Contract.set(
      MockAdmin.reserved_info(destination)
    ).to(uint(0));
  ...
  function distributeReservedTokens(uint num_destinations) internal view { 
    ...
    if (Contract.read(MockAdmin.is_final()) == bytes32(0))
      revert('crowdsale is not finalized');

    // Get total tokens sold, total token supply, and reserved destinations list length
    uint total_sold = uint(Contract.read(MockAdmin.tokens_sold()));
    uint total_supply = uint(Contract.read(MockAdmin.token_total_supply()));
    uint reserved_len = uint(Contract.read(MockAdmin.reserved_destinations()));
    ...
    for (uint i = 0; i < num_destinations; i++) {
      ...
      address addr = address(Contract.read(bytes32(32 * (num_destinations - i) + uint(MockAdmin.reserved_destinations())))); 

      // Get percent reserved and precision
      uint to_add = uint(Contract.read(
        bytes32(
          64 + uint(MockAdmin.reserved_info(addr))
        )
      ));
      ...
      uint precision = 2 + uint(Contract.read(
        bytes32(
          96 + uint(MockAdmin.reserved_info(addr))
        )
      ));
      ...
      to_add = to_add.add(uint(Contract.read(
        bytes32(
          32 + uint(MockAdmin.reserved_info(addr))
        )
      )));
  ...
  function distributeAndUnlockTokens() internal view { 
    ...
    for (uint i = 0; i < num_destinations; i++) {
      ...
      address reserved_address = address(Contract.read(
        bytes32(32 + (32 * i) + uint(MockAdmin.reserved_destinations())))
      );

      // Get percent reserved and precision
      uint to_add = uint(Contract.read(
        bytes32(64 + uint(MockAdmin.reserved_info(reserved_address)))
      ));

      // Two points of precision are added to ensure at least a percent out of 100
      uint precision = 2 + uint(Contract.read(
        bytes32(96 + uint(MockAdmin.reserved_info(reserved_address)))
      ));
      ...
      // Add number of tokens reserved
      to_add = to_add.add(uint(Contract.read(
        bytes32(32 + uint(MockAdmin.reserved_info(reserved_address)))
      )));
      ...
      // Add destination's current token balance to to_add
      to_add = to_add.add(uint(Contract.read(
        MockAdmin.balances(reserved_address)
      )));
  ...
  function finalizeAndDistributeToken() internal view { 
    ...
    for (uint i = 0; i < num_destinations; i++) {
      ...
      address reserved = address(Contract.read(bytes32(32 + (32 * i) + uint(MockAdmin.reserved_destinations()))));
      // Get percent reserved and precision
      uint to_add = uint(Contract.read(
        bytes32(64 + uint(MockAdmin.reserved_info(reserved)))
      ));
      // Two points of precision are added to ensure at least a percent out of 100
      uint precision = 2 + uint(Contract.read(
        bytes32(96 + uint(MockAdmin.reserved_info(reserved)))
      ));
      ...
      // Add number of tokens reserved
      to_add = to_add.add(uint(Contract.read(
        bytes32(32 + uint(MockAdmin.reserved_info(reserved)))
      )));
      // Increment total supply
      total_supply = total_supply.add(to_add);
      ...
      // Add destination's current token balance to to_add
      to_add = to_add.add(uint(Contract.read(
        bytes32(MockAdmin.reserved_info(reserved))
      )));
      // Store new destination token balance
      Contract.set(
        MockAdmin.balances(reserved)
      ).to(to_add);
    }
    // Update total supply
    Contract.set(
