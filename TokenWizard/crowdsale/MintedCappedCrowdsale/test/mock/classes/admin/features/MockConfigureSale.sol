...
library MockConfigureSale {
  ...
  function createCrowdsaleTiers(
    ...
    uint total_duration = uint(Contract.read(MockAdmin.total_duration()));
    uint num_tiers = uint(Contract.read(MockAdmin.crowdsale_tiers()));
    uint base_storage = 0;

    // Check that the sender is the crowdsale admin, and that the crowdsale is not initialized
    if (
      Contract.read(MockAdmin.is_init()) == bytes32(1)
      || address(Contract.read(MockAdmin.admin())) != Contract.sender()
  ...
  function whitelistMultiForTier(
    ...
    uint tier_whitelist_length = uint(Contract.read(MockAdmin.whitelist(tier_index)));
    ...
    for (uint i = 0; i < to_whitelist.length; i++) {
      ...
      bytes32 whitelist_status_loc = MockAdmin.address_whitelist(to_whitelist[i], tier_index);
      ...
      if (min_token_purchase[i] != 0 || max_wei_spend[i] != 0) {
        Contract.set(
          bytes32(32 + (32 * tier_whitelist_length) + uint(MockAdmin.whitelist(tier_index)))
        ).to(to_whitelist[i]);
    ...
    Contract.set(
      MockAdmin.whitelist(tier_index)
  ...
  internal pure { 
   // Ensure valid input
    ...
    Contract.set(MockAdmin.name()).to(name);
    Contract.set(MockAdmin.symbol()).to(symbol);
    Contract.set(MockAdmin.decimals()).to(decimals);
  ...
  internal view { 
    ...
    uint starts_at = uint(Contract.read(MockAdmin.start_time()));
    uint current_tier = uint(Contract.read(MockAdmin.current_tier()));
    uint total_duration = uint(Contract.read(MockAdmin.total_duration()));
    uint ends_at = uint(Contract.read(MockAdmin.ends_at()));
    uint previous_duration = uint(Contract.read(
      bytes32(128 + (192 * tier_index) + uint(MockAdmin.crowdsale_tiers())))
    );
    ...
    if (
      address(Contract.read(MockAdmin.admin())) != Contract.sender()
      || Contract.read(MockAdmin.is_final()) == bytes32(1)                  
      || uint(Contract.read(MockAdmin.crowdsale_tiers())) <= tier_index 
      || current_tier > tier_index 
      || (current_tier == tier_index 
         && tier_index != 0)
      || Contract.read(bytes32(160 + (192 * tier_index) + uint(MockAdmin.crowdsale_tiers()))) == 0
    ...
    if (tier_index == 0 && current_tier == 0) {
      ...
      for (uint i = current_tier; i < tier_index; i++)
        ends_at = ends_at.add(uint(Contract.read(bytes32(128 + (192 * i) + uint(MockAdmin.crowdsale_tiers())))));
