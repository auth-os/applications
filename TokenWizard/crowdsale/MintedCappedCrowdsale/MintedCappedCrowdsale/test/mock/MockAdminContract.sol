...
contract MockAdminContract {
  ...
  function advanceToTier(uint _tier_index) public pure returns (bytes memory) {
    // Create memory buffer for return data
    uint ptr = ptr.clear();

    ptr.stores();

    ptr.store(_tier_index).at(CROWDSALE_CURRENT_TIER);

    return ptr.getBuffer();
  }

  // MOCK FUNCTION - used to set the current tier's remaining tokens
  function setTierTokensRemaining(uint _val) public pure returns (bytes memory) {
    // Create memory buffer for return data
    uint ptr = ptr.clear();

    ptr.stores();

    ptr.store(_val).at(CURRENT_TIER_TOKENS_REMAINING);
