...
library ManageTokens {
  ...
  function initCrowdsaleToken(bytes32 _name, bytes32 _symbol, uint _decimals) internal pure {
   // Ensure valid input
    ...
    Contract.set(TokenManager.name()).to(_name);
    Contract.set(TokenManager.symbol()).to(_symbol);
    Contract.set(TokenManager.decimals()).to(_decimals);
  ...
  function setTransferAgentStatus(address _agent, bool _is_agent) internal pure {
    ...
    bytes32 status_location = TokenManager.transfer_agent(_agent);
    // Store new transfer agent status
    Contract.set(status_location).to(_is_agent);
  ...
  function updateMultipleReservedTokens(address[] _destinations, uint[] _num_tokens, uint[] _num_percents, uint[] _percent_decimals)
  internal view {
    // Ensure valid input
    if (
    ...
    uint num_destinations = uint(Contract.read(TokenManager.reserved_destinations()));
    ...
    for (uint i = 0; i < _destinations.length; i++) {
      ...
      if (Contract.read(TokenManager.reserved_info(_destinations[i])) == 0) {
        ...
        Contract.set(
          bytes32(32 * num_destinations + uint(TokenManager.reserved_destinations()))
        ).to(to_add);
        // Store reservation info
        Contract.set(
          TokenManager.reserved_info(to_add)
        ).to(num_destinations);
      }

      // Store reservation info
      Contract.set(
        bytes32(32 + uint(TokenManager.reserved_info(to_add)))
      ).to(_num_tokens[i]);
      Contract.set(
       bytes32(64 + uint(TokenManager.reserved_info(to_add)))
      ).to(_num_percents[i]);
      Contract.set(
        bytes32(96 + uint(TokenManager.reserved_info(to_add)))
      ).to(_percent_decimals[i]);
    }
    // Finally, update array length
    Contract.set(
      TokenManager.reserved_destinations()
    ).to(num_destinations);
  }
  ...
  function removeReservedTokens(address _destination) internal view {
    ...
    uint reservation_len = uint(Contract.read(TokenManager.reserved_destinations()));
    // Get index of passed-in destination. If zero, sender is not in reserved list - revert
    uint to_remove = uint(Contract.read(TokenManager.reserved_info(_destination)));
    ...
    if (to_remove != reservation_len) {
      ...
      address last_index = address(Contract.read(bytes32(32 * reservation_len + uint(TokenManager.reserved_destinations()))));

      // Update index
      Contract.set(
        TokenManager.reserved_info(last_index)
      ).to(to_remove);
      // Push last index address to correct spot in reserved_destinations() list
      Contract.set(
        bytes32((32 * to_remove) + uint(TokenManager.reserved_destinations()))
      ).to(last_index);
    }
    ...
    Contract.set(
      TokenManager.reserved_info(_destination)
    ).to(uint(0));
  }
  ...
  function distributeReservedTokens(uint _num_destinations) internal view {
    ...
    uint total_sold = uint(Contract.read(TokenManager.tokens_sold()));
    uint total_supply = uint(Contract.read(TokenManager.token_total_supply()));
    uint reserved_len = uint(Contract.read(TokenManager.reserved_destinations()));
    ...
    for (uint i = 0; i < _num_destinations; i++) {
      ...
      to_add = to_add.add(uint(Contract.read(
        bytes32(32 + uint(TokenManager.reserved_info(addr)))
      )));
    ...
    // Update total supply
    Contract.set(
      TokenManager.token_total_supply()
    ).to(total_supply);
  }
  ...
  function finalizeCrowdsaleAndToken() internal view {
    ...
    // Finalize crowdsale
    Contract.set(TokenManager.is_final()).to(true);
  ...
  function distributeAndUnlockTokens() internal view {
    ...
    if (num_destinations == 0) {
      // Unlock tokens
      Contract.set(TokenManager.tokens_unlocked()).to(true);
      return;
    }
    ...
    for (uint i = 0; i < num_destinations; i++) {
      ...
      address reserved_address = address(Contract.read(
        bytes32(32 + (32 * i) + uint(TokenManager.reserved_destinations())))
      );

      // Get percent reserved and precision
      uint to_add = uint(Contract.read(
        bytes32(64 + uint(TokenManager.reserved_info(reserved_address)))
      ));

      // Two points of precision are added to ensure at least a percent out of 100
      uint precision = 2 + uint(Contract.read(
        bytes32(96 + uint(TokenManager.reserved_info(reserved_address)))
      ));
      ...
      // Add number of tokens reserved
      to_add = to_add.add(uint(Contract.read(
        bytes32(32 + uint(TokenManager.reserved_info(reserved_address)))
      )));
      ...
      // Add destination's current token balance to to_add
      to_add = to_add.add(uint(Contract.read(
        TokenManager.balances(reserved_address)
      )));
  ...
  function finalizeAndDistributeToken() internal view {
    ...
    for (uint i = 0; i < num_destinations; i++) {
      ...
      address reserved = address(Contract.read(bytes32(32 + (32 * i) + uint(TokenManager.reserved_destinations()))));
      // Get percent reserved and precision
      uint to_add = uint(Contract.read(
        bytes32(64 + uint(TokenManager.reserved_info(reserved)))
      ));
      // Two points of precision are added to ensure at least a percent out of 100
      uint precision = 2 + uint(Contract.read(
        bytes32(96 + uint(TokenManager.reserved_info(reserved)))
      ));
      ...
      // Add number of tokens reserved
      to_add = to_add.add(uint(Contract.read(
        bytes32(32 + uint(TokenManager.reserved_info(reserved)))
      )));
      // Increment total supply
      total_supply = total_supply.add(to_add);
      ...
      // Add destination's current token balance to to_add
      to_add = to_add.add(uint(Contract.read(
        bytes32(TokenManager.reserved_info(reserved))
      )));
      // Store new destination token balance
      Contract.set(
        TokenManager.balances(reserved)
      ).to(to_add);
    }
    // Update total supply
    Contract.set(
