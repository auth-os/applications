...
contract('#TestRegistryInit', function(accounts) {
  ...
  before(async () => {
    ...
    testAddr = [
      registration.address, registration.address, completion.address, completion.address
    ]
...
/*    testProxy = await TestRegistryProxy.new(
      storage.address, regExecID, regProvider.address, appName,
      { from:proxyAdmin }
    ).should.be.fulfilled
    await testProxy.init({ from:proxyAdmin }).should.be.fulfilled*/
  })
  ...
  describe('registration', async () => {
    ...
    let initial
    let description = 'This is a test to test the register test function'
    ...
    describe('the valid registration', async () => {
      ...
      it('should have emitted two events', async () => {
        let calldata = await testUtils.registerTest.call(name, initial, description).should.be.fulfilled
    ...
    describe('the invalid registration', async () => {

      let invalidName = Utils.BYTES32_EMPTY 
      let invalidInitial = Utils.ADDRESS_0x
      let invalidDescription = ''

      it('should throw with \'invalid test name\'', async () => {
        await testProxy.registerTest(invalidName, initial, description).should.not.be.fulfilled
      })

      it('should throw with \'invalid initial\'', async () => {
        await testProxy.registerTest(name, invalidInitial, description).should.not.be.fulfilled
      })

      it('should throw with \'invalid description\'', async () => {
        await testProxy.registerTest(name, initial, invalidDescription).should.not.be.fulfilled
      })
